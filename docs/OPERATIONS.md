...
EventDrop'ta eventler ve fotograflar otomatik olarak expire olmaz ve otomatik
silinmez. Silme islemleri admin panelinden manuel yapilir.

Admin panelinden event silindiginde:

- Event'e bagli upload kayitlari okunur
- Guvenli storage path bilgisi olan dosyalar Supabase Storage'dan silinir
- Upload metadata kayitlari temizlenir
- Event kaydi silinir

Tekil fotograf silme de admin veya ilgili event icin yetkili misafir aksiyonu
ile yapilir. Misafir silmesi yalnizca album ayarlarinda acik ise calisir.

`/api/cleanup` endpoint'i otomatik silme yapmaz. Endpoint, eski health-check
akisinin disariya kapali oldugunu dogrulayabilmesi icin `CRON_SECRET` ile
korunan guvenli bir no-op olarak kalir.

## Future Operational Enhancements

- Host icin retention suresini etkinlik bazinda ayarlama
- Manuel album kapatma veya arsivleme
- Toplu indirme linki
